...
uses
  ...
  ComCtrls, ExtCtrls, StdCtrls, ActnList, Menus, XMLPropStorage, RTTICtrls,
  u_xml_xplplugin, v_class_combo, frm_template, v_msgbody_stringgrid,
  MStringGrid, MEdit, u_xpl_body;

type

  { TfrmPluginViewer }

  TfrmPluginViewer = class(TFrmTemplate)
    acOpenFile: TAction;
...
var frmPluginViewer: TfrmPluginViewer;
...
var
  ...
  i, j: integer;
begin
   OpenDialog1.InitialDir := xPLApplication.Folders.PluginDir;
   tvPlugin.Items.Clear;
   if FilePath<>'' then Caption := FilePath else exit;
...
procedure TfrmPluginViewer.FormCreate(Sender: TObject);
begin
  inherited;
  acOpenFile.ImageIndex:=K_IMG_DOCUMENT_OPEN;
...
procedure TfrmPluginViewer.acOpenFileExecute(Sender: TObject);
begin
