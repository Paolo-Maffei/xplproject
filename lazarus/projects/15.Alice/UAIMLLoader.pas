...
implementation
  uses SysUtils,UVariables,cStrings;
    ...
    begin
      ...
      if result then begin
        ...
        while not ((Parser.CurPartType=ptEndTag) and (Parser.CurName='category')) do
          parser.Scan;
      if (PatternMatcher._count mod 5000)=0 then writeln('aimlloader' + Inttostr(PatternMatcher._count)+' categories...');
    end;
  ...
  procedure TAIMLLoader.load(filename:string);
    ...
    begin
      ...
      if findfirst(filename,0,search) =0 then
        repeat
          ...
          if not FileExists(name) then begin
            writeln('aimlloader : Warning: '+Name+' not found');
            continue;
          end;
          writeln('aimlloader : Loading '+Name);
      ...
      FindClose(search);
      parser.free;
      writeln('aimlloader : Done, loaded '+InttoStr(PatternMatcher._count)+' categories');
    end;
end.
