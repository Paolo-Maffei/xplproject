  ...
  function TBotLoader.BotElement:boolean;
    ...
    begin
      ...
      if AnsiSameStr(Parser.CurAttr.Value('enabled'),'false') then Begin
        writeln('botloader : bot '''+bot_id+''' disabled');
        skipElement(parser);
        exit;
      end;
      writeln('botloader : Loading bot '''+bot_id+'''');
//      Log.OpenChatLog(bot_ID);
      ...
      writeln('botloader : Loaded '+inttostr(numprops)+ ' properties.');
    end;
  function TBotLoader.SentenceSplitters:boolean;
    ...
    begin
      ...
      writeln('botloader : Loaded '+inttostr(count)+' sentence splitters');
    end;
  function TBotLoader.InputSubstitutions:boolean;
    ...
    begin
      ...
      writeln('botloader : Loaded '+inttostr(count)+' input substitutions');
    end;

  procedure TBotLoader.load(filename:string);
    begin
      if loaded then Begin
        writeln('botloader : A bot is aready loaded');
        exit; {don't load 2 bots at 1 time}
      end;
      writeln('botloader : Loading '+filename+'...');
      ...
      parser.clear;
      parser.free;
      writeln('botloader : Done.');
      writeln('botloader' + inttostr(Nodecount));
    end;
