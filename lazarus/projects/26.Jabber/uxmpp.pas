...
uses
  ...
  LCLIntf, LCLType,
  {$ENDIF}
  Classes, SysUtils, fpTimer, tcpsynapse, xmltag;
...
begin
  ...
  if pty='error' then Exit;

  p  := tag.GetFirstTag('show');
  if p = nil then show :='' else show:= p.Data;
  ...
  begin
    if (show='') or (show='chat') and Assigned(FOnJoinedRoom) then FOnJoinedRoom(self,pf)
    else if Assigned(FOnLeftRoom) then FOnLeftRoom(self,pf);
  end;
end;
...
function TXmpp.IsConnected: boolean;
begin
