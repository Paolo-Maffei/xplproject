...
implementation
  uses SysUtils,UVariables,UUtils, frm_Main;
  ...
  function TBotLoader.BotElement:boolean;
    ...
    begin
      ...
      if AnsiSameStr(Parser.CurAttr.Value('enabled'),'false') then Begin
        frmMain.Log('botloader : bot '''+bot_id+''' disabled');
        skipElement(parser);
        exit;
      end;
      frmMain.Log('botloader : Loading bot '''+bot_id+'''');
//      Log.OpenChatLog(bot_ID);
      ...
      frmMain.log('botloader : Loaded '+inttostr(numprops)+ ' properties.');
    end;
  function TBotLoader.SentenceSplitters:boolean;
    ...
    begin
      ...
      frmMain.Log('botloader : Loaded '+inttostr(count)+' sentence splitters');
    end;
  function TBotLoader.InputSubstitutions:boolean;
    ...
    begin
      ...
      frmMain.Log('botloader : Loaded '+inttostr(count)+' input substitutions');
    end;

  procedure TBotLoader.load(filename:string);
    begin
      if loaded then Begin
        frmMain.Log('botloader : A bot is aready loaded');
        exit; {don't load 2 bots at 1 time}
      end;
      frmMain.Log('botloader : Loading '+filename+'...');
