...
implementation
  uses SysUtils,UVariables,cStrings, frm_Main;
    ...
    begin
      ...
      if result then begin
        ...
        while not ((Parser.CurPartType=ptEndTag) and (Parser.CurName='category')) do
          parser.Scan;
      if (PatternMatcher._count mod 5000)=0 then frmMain.Log('aimlloader' + Inttostr(PatternMatcher._count)+' categories...');
    end;
  ...
  procedure TAIMLLoader.load(filename:string);
    ...
    begin
      ...
      if findfirst(filename,0,search) =0 then
        repeat
          ...
          if not FileExists(name) then begin
            frmMain.log('aimlloader : Warning: '+Name+' not found');
            continue;
          end;
          frmMain.log('aimlloader : Loading '+Name);
      ...
      FindClose(search);
      parser.free;
      frmMain.log('aimlloader : Done, loaded '+InttoStr(PatternMatcher._count)+' categories');
    end;
end.
