...
type
  ...
  public
    ...
    property CharMapping: TKEditCharMapping read FCharMapping write SetCharMapping;
...
begin
  ...
  J := 0;
  for I := 1 to Length(S) do if not (S[I] in [#9, #32]) then
  begin
    K := DigitToBin(S[I]);
    if K >= 0 then
    begin
...
begin
  ...
  Data.Canvas.Font.Height := Font.Height;
...
procedure TKCustomHexEditor.SetKeyMapping(Value: TKEditKeyMapping);
begin
  ...
  try
    ...
    begin
      ...
      try
        ...
        DragQueryPoint(THandle(Msg.wParam), {$IFDEF FPC}@{$ENDIF}Point);
