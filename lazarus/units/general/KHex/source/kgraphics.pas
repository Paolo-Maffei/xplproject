...
{$ELSE}
  Windows, Messages,
{$ENDIF}
  Classes, Forms, Graphics, Controls;
...
type
  ...
  public
    ...
    { Creates an alpha channel in according to Alpha and Gradient. }
    procedure AlphaFill(Alpha: Byte; BlendColor: TColor; Gradient, Translucent: Boolean);
    ...
    { Fills the bitmap's alpha channel by Value if there is no alpha channel yet. }
    procedure FillAlphaIfNone(Value: Byte);
  {$IFNDEF FPC}
    { Does nothing. }
...
{$ENDIF}
  ...
  { @abstract(Encapsulates the drag window) Drag window is top level window used for
    dragging with mouse. It displays some portion of associated control. It can
    be translucent under Windows. }
...
procedure DrawAlignedText(Canvas: TCanvas; const ARect: TRect;
  HAlign: TKTextHAlign; VAlign: TKTextVAlign; HPadding, VPadding: Integer;
...
{ Draws a rectangle to Canvas. The rectangle coordinates are given by Rect.
  ...
  by BackColor. If BackColor is clNone these gaps are not filled and the Brush
  appears transparent. }
...
function KColorRec(R, G, B, A: Byte): TKColorRec;
...
uses
  Math, SysUtils, Types, KControls, KFunctions
{$IFDEF FPC}
  , FPImage
{$ENDIF}
  ;
...
var
  ...
  function MeasureOrOutput(Y: Integer; Output: Boolean): Integer;
  const
    cWordBreaks = [#0, #9,  #32];
    cLineBreaks = [#10, #13];
    cCR = #10;
  ...
  var
    ...
    procedure TrimWhiteSpaces(var AText: {$IFDEF STRING_IS_UNICODE}PChar{$ELSE}PWideChar{$ENDIF}; var ALen: Integer);
    begin
      while (ALen > 0) and (Char(AText[0]) in (cWordBreaks + cLineBreaks)) do begin AText := @AText[1]; Dec(ALen) end;
      while (ALen > 0) and (Char(AText[ALen - 1]) in (cWordBreaks + cLineBreaks)) do Dec(ALen);
    end;

    function TextExtent(AText: {$IFDEF STRING_IS_UNICODE}PChar{$ELSE}PWideChar{$ENDIF}; ALen: Integer; Trim: Boolean = False): TSize;
    begin
      if Trim then
        TrimWhiteSpaces(AText, ALen);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            SlashPos := ALen;
            while (SlashPos > 0) and not (Char(AText[SlashPos]) in ['/', '\']) do
              Dec(SlashPos);
            if SlashPos > 0 then
            begin
              DrawEllipsis := True;
  ...
  var
    ...
    WordBreak, LineBreak, WhiteSpace, PrevWhiteSpace, FirstWord: Boolean;
    Size: TSize;
  begin
    Vert := Y;
    if AText <> '' then
    begin
      ...
      begin
        ...
        begin
          ...
          WhiteSpace := AnsiChar(AText[I]) in (cWordBreaks + cLineBreaks);
          if not PrevWhiteSpace and WhiteSpace and (I > LineBegin) then
          begin
            if (LineBreaks = 0) and not FirstWord then
            begin
...
begin
  ...
  begin
    ...
    begin
      DC := Canvas.Handle;
      FontHeight := Canvas.TextHeight('A');
      case VAlign of
        valCenter:
          Y := Max(ClipRect.Top, (ClipRect.Bottom + ClipRect.Top - MeasureOrOutput(0, False)) div 2);
        valBottom:
          Y := ClipRect.Bottom - MeasureOrOutput(0, False);
      else
        Y := ClipRect.Top;
      end;
      TmpRect := ClipRect;
...
begin
  ...
  try
    I := CombineRgn(CurRgn, PrevRgn, RectRgn, Mode);
    Result := I <> NULLREGION;
    if Result then
...
procedure TKAlphaBitmap.FillAlphaIfNone(Value: Byte);
var
  I: Integer;
  HasAlpha: Boolean;
begin
