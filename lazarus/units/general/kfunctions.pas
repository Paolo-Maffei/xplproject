...
begin
  ...
  if (Digits = 0) or (Digits > 64) then
    Digits := 64;
  if (Len >= 1) and (S[Len] in ['b', 'B']) then
  begin
    Delete(S, Len, 1);
    Dec(Len);
  end;
...
begin
  ...
  if (Digits = 0) or (Digits > 16) then
    Digits := 16;
  if (Len >= 2) and (S[1] = '0') and (S[2] in ['x', 'X']) then
    I := 3
  else if (Len >= 1) and (S[1] in ['x', 'X', '$']) then
    I := 2
  else
    I := 1;
  while I <= Len do
