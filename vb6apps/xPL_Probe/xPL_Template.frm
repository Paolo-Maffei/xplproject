...
lookfornextmsg:
    ...
    If Right$(xPLCOMCmd, 1) <> vbCr Then Exit Sub
'    Debug.Print Now & " - " & xPLCOMCmd
    ...
    If Mid$(xPLCOMCmd, 3, 1) = "A" Then
        ...
        Probe = "all"
        If UCase(Mid$(xPLCOMCmd, 6, 1)) = "O" Then
            ' all on or all off
            Select Case UCase(Mid$(xPLCOMCmd, 6, 2))
            Case "ON"
                Status = "yyyyyy"
            Case "OF"
                Status = "nnnnnn"
        ...
        Else
            ...
            Status = "nnnnnn"
            For x = 1 To Len(str)
                Mid$(Status, Val(Mid(str, x, 1)), 1) = "y"
            Next x
        End If
    Else
        ' single
