...
Private Sub xPLSys_Received(Msg As xPLMsg)
    ...
    If UCase(Msg.Schema) <> "DAEMON.BASIC" Then Exit Sub
    cmd = UCase(xPL_GetParam(Msg, "COMMAND", True))
    drv = Val(xPL_GetParam(Msg, "DRIVE", True))
    path = ""
    For x = 0 To Msg.NamePairs
        If UCase(Msg.Names(x)) = "FOLDER" Then path = path & Msg.Values(x) & "\"
    Next x
    file = xPL_GetParam(Msg, "FILE", True)
    Select Case cmd
    Case "MOUNT"
        If drv < 1 Or drv > 4 Then GoTo failed
        drv = drv - 1
        If path = "" Then GoTo failed
        path = Replace(path, "/", "\")
        If Right(path, 1) <> "\" Then path = path & "\"
        If file = "" Then GoTo failed
        If Dir(path & file) = "" Then GoTo failed
        str = "-mount " & drv & "," & Chr$(34) & path & file & Chr$(34)
    Case "UNMOUNT"
        If drv < 1 Or drv > 4 Then GoTo failed
        drv = drv - 1
        str = "-unmount " & drv
    ...
    res = Shell(App.path & "\daemon.exe " & str, vbNormalFocus)
    On Error GoTo 0
...
Private Sub Form_Load()
    ...
    xPL_Source = "TONYT-DAEMON" ' set vendor-device here @@@
    ...
    xPL_Ready = False
    xPL_Title = "xPL Daemon" ' application title @@@
    Me.Caption = xPL_Title + " " + xPL_Source
