...
Public xPL_BridgeLocal As Long
Public xPL_BridgeRemote As Long
Public xPL_BridgeIP As String

Public oMD5 As New CMD5
Public oBlowfish As New clsBlowfish

Public Type MD5Cache
    Date As Date
    MD5 As String
End Type

Public xPL_Cache() As MD5Cache
Public xPL_CacheCount As Integer

Public xPL_Blowfish As String
...
' routine to get a parameter
Public Function xPL_GetParam2(inBody As Boolean, strName As String, WithStrip As Boolean) As Variant
    ...
    For y = 0 To xPL_Message(x).DC
        If UCase(xPL_Message(x).Details(y).Name) Like UCase(strName) Then
            ' got match
            xPL_GetParam2 = xPL_Message(x).Details(y).Value
            If WithStrip = True Then xPL_GetParam2 = Trim(xPL_GetParam2)
            Exit Function
        End If
    Next y
