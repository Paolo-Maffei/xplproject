...
Public Type RedRat
    ...
    IRCode As String
End Type
Public RedRat2() As RedRat
Public RedRatCodes As Integer
Public Learning As Boolean
Public LearnCancel As Boolean
Public RedRatPath As String
...
Public Sub LoadIRDatabase()
    ...
    Dim x As Integer
    Dim strRedRat As String
    Dim strRedRatLoad As RedRat

    ' load code database
    RedRatCodes = 0
    RedRatPath = App.Path
    If Right$(RedRatPath, 1) <> "\" Then RedRatPath = RedRatPath + "\"
    RedRatPath = RedRatPath + "RedRat.cfg"
    If Dir(RedRatPath, vbNormal) <> "" Then
        ' got database, so load
        Open RedRatPath For Input As #1
        While Not EOF(1)
            Line Input #1, strRedRat
            x = InStr(1, strRedRat, ",", vbBinaryCompare)
            If x > 1 Then
                ' got device
                strRedRatLoad.Device = Left$(strRedRat, x - 1)
                strRedRat = Mid$(strRedRat, x + 1)
                x = InStr(1, strRedRat, ",", vbBinaryCompare)
                If x > 1 Then
                    ' got button
                    strRedRatLoad.Button = Left$(strRedRat, x - 1)
                    strRedRat = Mid$(strRedRat, x + 1)
                    If Len(strRedRat) > 10 Then
                        ' got it code
                        strRedRatLoad.IRCode = strRedRat
                        ' load it up
                        RedRatCodes = RedRatCodes + 1
                        ReDim Preserve RedRat2(RedRatCodes)
                        RedRat2(RedRatCodes).Device = strRedRatLoad.Device
                        RedRat2(RedRatCodes).Button = strRedRatLoad.Button
                        RedRat2(RedRatCodes).IRCode = strRedRatLoad.IRCode
                    End If
                End If
            End If
        Wend
