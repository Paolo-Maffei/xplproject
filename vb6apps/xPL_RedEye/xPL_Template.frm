...
Private Sub xPLSys_Received(Msg As xPL.xPLMsg)
    ...
    If UCase(xPL_GetParam(Msg, "ZONE", True)) = UCase(Mid$(xPL_Source, x + 1)) Then
        RedEye(0).Device = xPLSys.Configs("DEVICE") ' device
        Delay = Val(xPL_GetParam(Msg, "DELAY", True))  ' delay
        Keys = xPL_GetParam(Msg, "KEYS", True) ' keys
        If RedEye(0).Device <> "" And Keys <> "" Then
            ' got something to send
            Me.txtMsg(1) = "Device=" + RedEye(0).Device + vbCrLf
            If Right$(Keys, 1) <> "," Then Keys = Keys + ","
            x = InStr(1, Keys, ",", vbBinaryCompare)
            While x > 0
                RedEye(0).Button = Left$(Keys, x - 1)
                TXIR = False
                If RedEyeCodes > 0 And RedEye(0).Button <> "" Then
                    For y = 1 To RedEyeCodes
                        If UCase(RedEye(y).Device) = UCase(RedEye(0).Device) And UCase(RedEye(y).Button) = UCase(RedEye(0).Button) Then
                            RedEye(0).IRCode = RedEye(y).IRCode
                            Me.txtMsg(1) = Me.txtMsg(1) + "Key=" + RedEye(0).Button + vbCrLf
                            TXIR = True
                            y = RedEyeCodes + 1
                        End If
                    Next y
                End If
                ' send
                If TXIR = True And RedEye(0).IRCode <> "" Then
                    Select Case Delay
...
Private Sub xPLSys_Configured(Source As String)
    ...
    While Wait > Now()
        DoEvents
    Wend
    Select Case UCase(xPLSys.Configs("DEVICE"))
    Case "PACE_IRDA"
        xPLCOM.Output = "%"
    Case "PACE_RC5"
        xPLCOM.Output = "&"
...
Private Sub Form_Load()
    ...
    xPL_Source = "TONYT-REDEYE" ' set vendor-device here @@@
    ...
    xPL_Ready = False
    xPL_Title = "RedEye" ' application title @@@
    Me.Caption = xPL_Title + " " + xPL_Source
    ...
    Call xPLSys.ConfigsAdd("DEVICE", "RECONF", 1)
...
'    etc
    ...
    xPLSys.Configs("DEVICE") = "PACE_IRDA"
'    etc
