...
Private Sub xPLSys_Received(Msg As xPLMsg)
    ...
    strPath = xPL_GetParam(Msg, "PATH", True)
    strProg = xPL_GetParam(Msg, "APPLICATION", True)
    strStyle = UCase(xPL_GetParam(Msg, "STYLE", True))
    strFocus = UCase(xPL_GetParam(Msg, "FOCUS", True))
    If strPath <> "" Then
        strShell = strPath
        If Right$(strShell, 1) <> "\" Then strShell = strShell & "\"
    End If
    strShell = strShell & strProg
    Select Case strStyle
    Case "HIDDEN"
        z = vbHide
    Case "NORMAL"
        z = vbNormalNoFocus
        If strFocus = "YES" Then z = vbNormalFocus
    Case "MINIMIZED"
        z = vbMinimizedNoFocus
        If strFocus = "YES" Then z = vbMinimizedFocus
    Case "MAXIMISED"
        z = vbMaximizedFocus
    Case Else
        z = vbNormalNoFocus
    ...
    On Error GoTo 0
    If x = 0 Then
        Call xPLSys.SendXplMsg("xpl-trig", Msg.Source, "shell.basic", "status=FAILED" & Chr$(10) & "application=" & strProg)
    Else
        Call xPLSys.SendXplMsg("xpl-trig", Msg.Source, "shell.basic", "status=OK" & Chr$(10) & "application=" & strProg)
    End If
    Exit Sub
...
Private Sub Form_Load()
    ...
    xPL_Source = "TONYT-LAUNCH" ' set vendor-device here @@@
    ...
    xPL_Ready = False
    xPL_Title = "Launcher" ' application title @@@
    Me.Caption = xPL_Title + " " + xPL_Source
...
'    etc
    ...
    ' add default filters @@@
    Call xPLSys.FiltersAdd("xpl-cmnd.*.*.*.shell.basic")
    ' etc
