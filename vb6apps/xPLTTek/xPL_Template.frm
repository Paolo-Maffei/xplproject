...
Private Sub xPLSys_Received(Msg As xPLMsg)
    ...
    If Msg.Target = "*" Then Exit Sub ' no broadcasts
    If UCase(Msg.Schema) <> "TTEK.BASIC" Then Exit Sub
    ...
    Case "CHAPTER"
        tcmd = 32798
    End Select
    If tcmd <> 0 Then SendCommand (tcmd)
...
Private Sub Form_Load()
    ...
    xPL_Source = "TONYT-TTEK" ' set vendor-device here @@@
    ...
    xPL_Ready = False
    xPL_Title = "xPL TheaterTek" ' application title @@@
    Me.Caption = xPL_Title + " " + xPL_Source
    Me.lblxPL(0) = "xPL RX" ' receive box label @@@
    Me.lblxPL(1) = "Command" ' receive box label @@@
    Me.mPopRestore.Caption = xPL_Source
