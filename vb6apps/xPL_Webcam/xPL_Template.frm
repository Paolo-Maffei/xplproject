...
Begin VB.Form xPL_Template 
   ...
   Begin VB.TextBox txtMsg 
      ...
      Width           =   5655
   End
   Begin InetCtlsObjects.Inet xPLInet 
      Left            =   4560
      Top             =   4560
      _ExtentX        =   1005
      _ExtentY        =   1005
...
Private Sub xPLSys_Received(Msg As xPLMsg)
    ...
    Case "IMAGE"
        Me.txtMsg(1) = Format(Now(), "dd/mm/yyyy hh:mm:ss") + vbCrLf + vbCrLf
        UseURL = xPL_GetParam(Msg, "URL", True)
        If UseURL = "" Then UseURL = xPLSys.Configs("URL")
        UseFile = xPL_GetParam(Msg, "FILE", True)
        If UseFile = "" Then UseFile = xPLSys.Configs("SAVEAS")
        Me.txtMsg(1) = Me.txtMsg(1) + "URL: " + UseURL + vbCrLf
        Me.txtMsg(1) = Me.txtMsg(1) + "FILE: " + UseFile + vbCrLf + vbCrLf
        intFile = FreeFile()
        On Error GoTo connectfail
        byteData() = xPLInet.OpenURL(UseURL, icByteArray)
        On Error GoTo openfilefail
        Open UseFile For Binary Access Write As #intFile
        On Error GoTo connectfail
        Put #intFile, , byteData()
        On Error GoTo 0
        Close #intFile
        Me.txtMsg(1) = Me.txtMsg(1) + "Succeeded"
    Case Else
        Me.txtMsg(1) = Format(Now(), "dd/mm/yyyy hh:mm:ss") + vbCrLf + vbCrLf + "Unsupported Command"
    End Select
    Exit Sub
    
connectfail:
    On Error GoTo 0
    Me.txtMsg(1) = Me.txtMsg(1) + "Connect to URL or Download Failed"
    Exit Sub

openfilefail:
    On Error GoTo 0
    Me.txtMsg(1) = Me.txtMsg(1) + "Open File for Output Failed"
...
Private Sub Form_Load()
    ...
    xPL_Source = "TONYT-WEBCAM" ' set vendor-device here @@@
    ...
    xPL_Ready = False
    xPL_Title = "Webcam Server Client" ' application title @@@
    Me.Caption = xPL_Title + " " + xPL_Source
    Me.lblxPL(0) = "xPL RX" ' receive box label @@@
    Me.lblxPL(1) = "Last Status" ' receive box label @@@
    Me.mPopRestore.Caption = xPL_Source
    ...
    Call xPLSys.ConfigsAdd("URL", "OPTION", 1)
    Call xPLSys.ConfigsAdd("SAVEAS", "OPTION", 1)
'    etc
...
'    etc
    ...
    ' add default filters @@@
    Call xPLSys.FiltersAdd("xpl-cmnd.*.*.*.webcam.basic")
    ' etc
