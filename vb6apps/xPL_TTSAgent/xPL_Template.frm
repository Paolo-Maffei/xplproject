...
Begin VB.Form xPL_Template 
   ...
   Begin VB.TextBox txtMsg 
      ...
      Width           =   3735
   End
   Begin AgentObjectsCtl.Agent TTSAgent 
      Left            =   0
      Top             =   0
   End
...
Private Sub xPLSys_Received(Msg As xPL.xPLMsg)
    ...
    If xPL_Text = "" Then xPL_Text = xPL_GetParam(Msg, "XPL-SPEECH", True)
    
    ' process tts message
    If xPL_Text <> "" And xPLSys.Configs("AGENT") <> "" Then
        TTS_Agent.Top = Val(xPLSys.Configs("TOP"))
        TTS_Agent.Left = Val(xPLSys.Configs("LEFT"))
        TTS_Agent.Show
        TTS_Agent.Speak xPL_Text
        TTS_Agent.Hide
    End If
...
Private Sub xPLSys_Configured(Source As String)
    ...
    On Error GoTo agentnotloaded
    TTSAgent.Characters.Load xPLSys.Configs("AGENT"), xPLSys.Configs("AGENT") + ".acs"
    On Error Resume Next
    Set TTS_Agent = TTSAgent.Characters(xPLSys.Configs("AGENT"))
    TTS_Agent.LanguageID = &H409
...
Private Sub Form_Load()
    ...
    xPL_Source = "TONYT-TTSAGENT" ' set vendor-device here @@@
...
'    Me.lblxPL(1) = "xPL TX" ' receive box label @@@
    ...
    Call xPLSys.ConfigsAdd("AGENT", "OPTION", 1)
    Call xPLSys.ConfigsAdd("ANIMATION", "OPTION", 1)
    Call xPLSys.ConfigsAdd("TOP", "OPTION", 1)
    Call xPLSys.ConfigsAdd("LEFT", "OPTION", 1)
    
'    etc

    ' add default extra config values if possible @@@
    xPLSys.Configs("AGENT") = "MERLIN"
    xPLSys.Configs("ANIMATION") = "ANNOUNCE"
    xPLSys.Configs("TOP") = "20"
    xPLSys.Configs("LEFT") = "20"
    
'    etc

    ' add default filters @@@
    Call xPLSys.FiltersAdd("xpl-cmnd.*.*.*.*.*")
    ' etc
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    ' tidy up stuff here @@@
    Set TTS_Agent = Nothing
    On Error Resume Next
    TTSAgent.Characters.Unload xPLSys.Configs("AGENT")
    On Error GoTo 0
