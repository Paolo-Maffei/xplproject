...
namespace Medusa.X10CM11
{
	...
	public class X10EventArgs:EventArgs
	{
		...
		private readonly X10_COMMANDS mX10Command;
	...
	public sealed class X10Comm:IDisposable
	{
		...
		private class X10Message
		{
			...
			private X10_COMMANDS mX10Command;
			...
			public byte Data1 { get { return mData1; }}
			public byte Data2 { get { return mData2; }}
			public bool IsExtended() { return (mX10Command ==  X10_COMMANDS.EXTENDED);}
				...
				while (mustWait)
				{
					...
					if (!mustWait)
					{
						...
						try
						{
							...
							if (x10m.X10Command ==  X10_COMMANDS.EXTENDED) //extended
							{
								extended[0] = 7;
							...
							else  //standard function
							{
								...
								standard[1] = (byte)((byte) (x10m.Code2Byte(x10m.HouseCode) << 4) | (byte) (x10m.X10Command));
			...
			private void TreatBuffer()
			{
				for (int i=1; i<bufsize; i++)
				{
					if (((buffer[0]>>(i-1)) & 1)==1)  //function
					{
						...
						if (func == (byte) (X10_COMMANDS.DIM) || func == (byte) (X10_COMMANDS.BRIGHT))  //dim or bright  -> next is brightness
						{
							i++;
					...
					if (havefunc)
					{
						instance.RaiseReceivedX10(new X10EventArgs(house.ToString() + addresses.TrimEnd(new char[] {','}), (X10_COMMANDS) (func), bright, d1, d2));
						//reset values
						havefunc = false;
