...
'* Version 1.52
'*
'* Copyright (C) 2003-2006 John Bent, Ian Jeffery & Tony Tofts
'* http://www.xplhal.com/
'*
...
Public Class xplDeterminator
    ...
    Public Class DeterminatorCondition
      ...
      Public Class x10Condition
        ...
        Public Function Evaluate() As Boolean          
          Dim x, y As Integer
          x = Asc(Left(Device, 1)) - 64
          y = CInt(Mid(Device, 2))
          Dim currentState As Short = X10Cache(x, y).State

              Select Case state
                Case "ON"
              If currentState = X10_ON Then
                Return True
              End If
            Case "OFF"
              If currentState = X10_OFF Then
                Return True
              End If
          End Select
          Return False
      ...
      Public Class dayCondition
        ...
        Public Function Evaluate() As Boolean
          ...
          Else
            If xhcp.EnableLogging Then
              WriteErrorLog("Length of DOW should be 7 and it's actually " & DOW.Length)
            End If
          End If
          Return False
        End Function
            ...
            Case "day"
              ...
              Select Case [Operator]
                ...
                Case ">"
                  ...
                  End If
                Case Else
                  WriteErrorLog("Unrecognised operator for timeCondition: " & [Operator])
              End Select
            Case "month"
              DateValue = CInt(value)
              Select Case [Operator]
      ...
      Public Class xplCondition
        ...
        Public Function Evaluate(ByVal x As xpllib.XplMsg) As Boolean
          If xhcp.EnableLogging Then
            WriteErrorLog("Checking xPL message against our conditions.")
          End If
          ' Check type of message
          If msg_type <> "*" Then
            If ("XPL-" & msg_type) <> x.XPL_Msg(0).Section Then
              If xhcp.EnableLogging Then
                WriteErrorLog("Message types don't match. We're looking for XPL-" & msg_type & " and we've got " & x.XPL_Msg(0).Section)
              End If
              Return False
            End If
          End If
          ...
          If Not source_vendor = "*" Then
            If Not source_vendor = x.Source.Vendor.ToLower Then
              If xhcp.EnableLogging Then
                WriteErrorLog("Source vendor does not match: " & source_vendor & "=" & x.Source.Vendor.ToLower)
              End If
              Return False
            End If
          End If
          If Not source_device = "*" Then
            If Not source_device = x.Source.Device Then
              If xhcp.EnableLogging Then
                WriteErrorLog("Source device does not match.")
              End If
              Return False
            End If
          End If
          If Not source_instance = "*" Then
            If Not source_instance = x.Source.Instance Then
              If xhcp.EnableLogging Then
                WriteErrorLog("Source instance does not match: " & source_instance & "=" & x.Source.Instance)
              End If
              Return False
            End If
          End If
          ...
          If Not target_vendor = "*" Then
            If Not target_vendor = x.Target.Vendor Then
              If xhcp.EnableLogging Then
                WriteErrorLog("Target vendor does not match.")
              End If
              Return False
            End If
          End If
          If Not target_device = "*" Then
            If Not target_device = x.Target.Device Then
              If xhcp.EnableLogging Then
                WriteErrorLog("Target device does not match.")
              End If
              Return False
            End If
          End If
          If Not target_instance = "*" Then
            If Not target_instance = x.Target.Instance Then
              Return False
            End If
          End If
          ' Check the schema
          If Not schema_class = "*" Then
            If Not schema_class = x.Schema.msgClass Then
              If xhcp.EnableLogging Then
                WriteErrorLog("Schema class does not match.")
              End If
              Return False
            End If
          End If
          If Not schema_type = "*" Then
            If Not schema_type = x.Schema.msgType Then
              If xhcp.EnableLogging Then
                WriteErrorLog("Schema type does not match.")
              End If
              Return False
            End If
          End If
          ...
          For Counter As Integer = 0 To params.Length - 1
            Select Case params(Counter).[Operator]
              Case "="
                If Not x.GetParam(1, params(Counter).Name).Trim.ToLower = params(Counter).Value Then
                  If xhcp.EnableLogging Then
                    WriteErrorLog("Parameter " & x.GetParam(1, params(Counter).Name).ToLower & "=" & params(Counter).Value & " is not true.")
                  End If
                  Return False
                End If
            End Select
      ...
      Public Class globalCondition
        ...
        Public Function Evaluate() As Boolean
          Try
            Name = Name.ToUpper
            If Not xPLGlobals.Exists(Name) Then
              Return False
            End If

            ' If the value is a reference to a global, grab the value
            If Value.StartsWith("{") And Value.IndexOf("}") > 1 Then
              Dim globalName As String = Value.Substring(1, Value.IndexOf("}") - 1).ToUpper()
              If xPLGlobals.Exists(globalName) Then
                Value = xPLGlobals.Value(globalName).ToString              
              End If
            End If
            Select Case [Operator]
              Case "="
                If xPLGlobals.Value(Name).ToString = Value Then
                  Return True
                Else
                  Return False
                End If
              Case "!="
                ...
                End If
              Case "<"
                If Not IsNumeric(xPLGlobals.Value(Name).ToString) Then
                  Return False
                End If
                If CDbl(xPLGlobals.Value(Name)) < CDbl(Value) Then
                  If xhcp.EnableLogging Then
                    WriteErrorLog("The value " & CDbl(xPLGlobals.Value(Name)) & " is less than " & CDbl(Value) & " - returning TRUE.")
                  End If
                  Return True
                Else
                  Return False
              ...
              Case ">="
                ...
                End If
              Case Else
                WriteErrorLog("Unrecognised global condition comparison operator: " & [Operator])
            End Select
          Catch ex As Exception
            WriteErrorLog("Error evaluating global condition: " & ex.Message)
            Evaluate = False
          End Try
        End Function
      End Class
      ...
      Public Class globalChanged
        ...
        Public Function Evaluate(ByVal name As String) As Boolean
          If name.ToUpper = globalName.ToUpper Then
            Return True
          Else
            Return False
          End If
    ...
    Public Class DeterminatorAction
      ...
      Public Class RunScriptAction
        ...
        Public Sub Execute()
          Dim res As Boolean
          If Parameters = "" Then
            res = RunScript(ScriptName, False, "")
          Else
            res = RunScript(ScriptName, True, Parameters)
          End If
          If Not res Then
            WriteErrorLog("Script " & ScriptName & " failed to execute.")
          End If
        End Sub
      End Class
      ...
      Public Class executeAction
        ...
        Public Sub Execute()
          Try
            If xhcp.EnableLogging Then
              WriteErrorLog("Executing " & Program & " with parameters " & Parameters)
            End If
            ...
            p.Close()
          Catch ex As Exception
            WriteErrorLog("Error executing program " & Program & ": " & ex.Message)
          End Try
        End Sub
      ...
      Public Class xplAction
        ...
        Public Sub Execute(ByRef xSource As xpllib.XplMsg)
          ...
          str &= "hop=1" & vbLf & "source=" & xPLHalSource & vbLf
          ...
          For Counter As Integer = 0 To params.Length - 1
            If params(Counter).IndexOf("{") > 0 Then
              ...
              For Each globalname As String In xPLGlobals.Globals.Keys
                newParams(Counter) = newParams(Counter).Replace("{" & globalname & "}", xPLGlobals.Globals(globalname).ToString)
              Next
              ' Loop through source parameters
              If Not xSource Is Nothing Then
                For Counter2 As Integer = 0 To xSource.XPL_Msg(1).DC
                  newParams(Counter) = newParams(Counter).Replace("{XPL::" & xSource.XPL_Msg(1).Details(Counter2).keyName.ToUpper & "}", xSource.XPL_Msg(1).Details(Counter2).Value)
                Next
              End If
            End If
      ...
      Public Class globalAction
        ...
        Public Sub Execute(ByRef xSource As xpllib.XplMsg)
          ...
          Name = Name.ToUpper
          If xhcp.EnableLogging Then
            WriteErrorLog("Setting global " & Name & " to " & newValue)
          End If
          Try
            ' De-reference any xPL body parameters
            If Not xSource Is Nothing And newValue.IndexOf("{XPL::") >= 0 Then
              For Counter2 As Integer = 0 To xSource.XPL_Msg(1).DC
                newValue = newValue.Replace("{XPL::" & xSource.XPL_Msg(1).Details(Counter2).keyName.ToUpper & "}", xSource.XPL_Msg(1).Details(Counter2).Value)
              Next
            End If

            ' If it's a setting (i.e. mode or period) then treat it differently
            If xPLHalsHash.ContainsKey(Name) Then
              Dim Counter, i As Integer
              newValue = newValue.ToUpper
              i = CInt(xPLHalsHash(Name))
              For Counter = 0 To xPLHals(i).ValuesCount
                If xPLHals(i).Values(Counter).Name.ToUpper() = newValue Then
                  SYSClass.Setting(Name) = Counter
                End If
              Next
            Else
              ...
              If newValue.StartsWith("{") And newValue.IndexOf("}") > 1 Then
                Dim global2 As String = newValue.Substring(1, newValue.IndexOf("}") - 1).ToUpper
                If xhcp.EnableLogging Then
                  WriteErrorLog("Global2=" & global2 & ".")
                End If
                If newValue.EndsWith("++") Then
                  ' Increment global
                  SYSClass.Value(Name) = CDbl(SYSClass.Value(global2)) + 1
                ...
                ElseIf global2.StartsWith("SYS::") Then
                  ' Replace system variables
                  If xhcp.EnableLogging Then
                    WriteErrorLog("Replacing system variables in " & newValue)
                  End If
                  newValue = ProcessSystemVariables(newValue)
                  SYSClass.Value(Name) = newValue
                Else
                  SYSClass.Value(Name) = SYSClass.Value(global2)
                End If
              Else
            ...
            End If
          Catch ex As Exception
            WriteErrorLog("Error setting global " & Name & ": " & ex.Message)
          End Try
        End Sub
      End Class
        ...
        Try
          ...
          Select Case ActionType
            ...
            Case ActionTypes.xplAction
              CType(Action, xplAction).Execute(xSource)
          End Select
        Catch ex As Exception
          WriteErrorLog("Error executing action: " & DisplayName & ": " & ex.ToString)
        End Try
      End Function
  ...
  Public Sub New()
    ReDim Rules(-1)
    ReDim RuleGroups(-1)
    Directory.CreateDirectory(xPLHalData & "\Determinator")
    LoadRules()
  End Sub

  Public Sub GlobalChanged(ByVal globalName As String)
    Try
      ' Loop through all rules
      For COunter As Integer = 0 To Rules.Length - 1
        ' Loop through all conditions
        For Counter2 As Integer = 0 To Rules(COunter).Conditions.Length - 1
          If Rules(COunter).Conditions(Counter2).ConditionType = DeterminatorRule.ConditionTypes.globalChanged Then
            If CType(Rules(COunter).Conditions(Counter2).Condition, DeterminatorRule.DeterminatorCondition.globalChanged).globalName = globalName.ToUpper Then
              ExecuteRule(Rules(COunter).RuleName)
              Exit For
            End If
          End If
        Next
      Next
    Catch ex As Exception
      WriteErrorLog("Error checking determinators following global value change: " & ex.Message)
    End Try
  End Sub
  ...
  Public Shared Sub ExecuteRule(ByVal rulename As String, Optional ByVal offset As Integer = 0, Optional ByVal RunIfDisabled As Boolean = True)
    ...
    For Counter As Integer = 0 To Rules.Length - 1
      If Not Rules(Counter).IsGroup And (Rules(Counter).RuleName.ToUpper = rulename Or Rules(Counter).RuleGUID.ToUpper = rulename) Then
        If xhcp.EnableLogging Then
          WriteErrorLog("Evaluating the rule " & rulename & " from offset " & offset)
        End If
        ' If disabled, don't run it if configured not to do so
        If Not RunIfDisabled And Not Rules(Counter).Enabled Then
          If xhcp.EnableLogging Then
            WriteErrorLog("Rule is disabled.")
          End If
          Exit Sub
        End If
        Dim res As Boolean = True, found As Boolean = False
        ' Evaluate any group conditions
        If Rules(Counter).GroupName <> "" Then

        End If

        ' Loop through all conditions and evaluate them
        If Rules(Counter).Conditions.Length = 0 Or offset > 0 Then
          ' No conditions defined, or we're resuming a suspended determinator
          found = True
        Else
          For Counter2 As Integer = 0 To Rules(Counter).Conditions.Length - 1
            Select Case Rules(Counter).Conditions(Counter2).ConditionType
              Case DeterminatorRule.ConditionTypes.dayCondition
                Dim x As DeterminatorRule.DeterminatorCondition.dayCondition = CType(Rules(Counter).Conditions(Counter2).Condition, DeterminatorRule.DeterminatorCondition.dayCondition)
                res = x.evaluate()
              Case DeterminatorRule.ConditionTypes.timeCondition
                Dim x As DeterminatorRule.DeterminatorCondition.timeCondition = CType(Rules(Counter).Conditions(Counter2).Condition, DeterminatorRule.DeterminatorCondition.timeCondition)
                res = x.evaluate()
              Case DeterminatorRule.ConditionTypes.x10Condition
                Dim x As DeterminatorRule.DeterminatorCondition.x10Condition = CType(Rules(Counter).Conditions(Counter2).Condition, DeterminatorRule.DeterminatorCondition.x10Condition)
                res = x.evaluate()
              Case DeterminatorRule.ConditionTypes.globalCondition
                Dim x As DeterminatorRule.DeterminatorCondition.globalCondition = CType(Rules(Counter).Conditions(Counter2).Condition, DeterminatorRule.DeterminatorCondition.globalCondition)
                res = x.Evaluate()
                If xhcp.EnableLogging Then
                  WriteErrorLog("Global condition " & x.Name & x.[Operator] & x.Value & " evaluated to " & res.ToString)
                End If
            End Select
            If Rules(Counter).MatchAny = False And res = False Then
              Exit Sub
            ElseIf res Then
              found = True
            End If
        ...
        If found Then
          If xhcp.EnableLogging Then
            WriteErrorLog("Executing rule: " & Rules(Counter).RuleName)
          End If
          For Counter2 As Integer = offset To Rules(Counter).Actions.Length - 1
            If Not Rules(Counter).Actions(Counter2).Execute(Rules(Counter).RuleName, Nothing) Then
              Exit For
            End If
          Next
          Exit For
        Else
          If xhcp.EnableLogging Then
            WriteErrorLog("Rule not executed.")
          End If
        End If
      End If
    Next
  ...
  Public Sub SaveRule(ByVal ruleGuid As String)
    ' Saves a rule or rule group to disk
    For Counter As Integer = 0 To Rules.Length - 1
      If Rules(Counter).RuleGUID = ruleGuid Then
        Dim RuleText As String = Rules(Counter).Save()
  ...
  Public Sub LoadRule(ByVal ruleGuid As String)
    ...
    Try
      ...
      For Counter = 0 To Rules.Length - 1
        If Rules(Counter).RuleGUID = ruleGuid Then
      ...
      Dim xml As New XmlTextReader(xPLHalData & "\Determinator\" & ruleGuid & ".xml")
          ...
          Case XmlNodeType.Element
            Select Case xml.Name
              ...
              Case "x10Condition"
                ...
                With newRule.Conditions(newRule.Conditions.Length - 1)
                  .ConditionType = DeterminatorRule.ConditionTypes.x10Condition
                  .DisplayName = xml.GetAttribute("display_name")
                  x.Device = xml.GetAttribute("device").ToUpper
                  x.State = xml.GetAttribute("state").ToUpper
                  .Condition = x
                End With
              Case "xplCondition"
                ReDim Preserve newRule.Conditions(newRule.Conditions.Length)
                ...
                With newRule.Conditions(newRule.Conditions.Length - 1)
                  ...
                  .DisplayName = xml.GetAttribute("display_name")
                  x.msg_type = xml.GetAttribute("msg_type").ToUpper
              ...
              Case "globalChanged"
                ...
                x.globalName = xml.GetAttribute("name").ToUpper
                With newRule.Conditions(newRule.Conditions.Length - 1)
                  .Condition = x
                  .DisplayName = xml.GetAttribute("display_name")
                End With
    ...
    Catch ex As Exception
      If newRule.RuleName Is Nothing Then
        Call WriteErrorLog("Error loading rule " & ruleGuid & ": " & ex.Message)
      Else
        Call WriteErrorLog("Error loading rule " & ruleGuid & ": " & ex.Message)
      End If
    End Try
  ...
  Public Function DeleteRule(ByVal ruleGuid As String, ByVal deleteFromFileSystem As Boolean) As Boolean
  ...
  Public Sub AddRule(ByVal r As DeterminatorRule)
    RulesMutex.WaitOne()
    ReDim Preserve Rules(Rules.Length)
    Rules(Rules.Length - 1) = r
    RulesMutex.ReleaseMutex()
    If xhcp.EnableLogging Then
      WriteErrorLog("Loaded rule: " & r.RuleName)
    End If
  End Sub
  ...
  Public Sub LoadRules()
    RulesMutex.WaitOne()
    'Try
    Dim files() As String = Directory.GetFiles(xPLHalData & "\Determinator")
    Dim f As String
  ...
  Public Sub ProcessXplMessage(ByVal x As xpllib.XplMsg)
    Dim m As New DeterminatorProcessor
    Dim t As New Thread(AddressOf m.Start)
    m.Message = x
    t.Start()
  End Sub
  ...
  Public Class DeterminatorProcessor
    ...
    Private Sub ProcessRule(ByVal ruleIndex As Integer)
      If xhcp.EnableLogging Then
        WriteErrorLog("Evaluating rule " & Rules(ruleIndex).RuleName & " following reception of an xPL message.")
      End If
      If Not Rules(ruleIndex).Enabled Then
        If xhcp.EnableLogging Then
          WriteErrorLog("Rule is disabled.")
        End If
        Exit Sub
      ...
      ' Loop through all conditions and evaluate them
      For Counter = 0 To Rules(ruleIndex).Conditions.Length - 1
        If xhcp.EnableLogging Then
          WriteErrorLog("Evaluating condition " & Rules(ruleIndex).Conditions(Counter).DisplayName)
        End If
        Select Case Rules(ruleIndex).Conditions(Counter).ConditionType
          Case DeterminatorRule.ConditionTypes.dayCondition
            Dim x As DeterminatorRule.DeterminatorCondition.dayCondition = CType(Rules(ruleIndex).Conditions(Counter).Condition, DeterminatorRule.DeterminatorCondition.dayCondition)
            res = x.evaluate()
          Case DeterminatorRule.ConditionTypes.globalCondition
            Dim x As DeterminatorRule.DeterminatorCondition.globalCondition = CType(Rules(ruleIndex).Conditions(Counter).Condition, DeterminatorRule.DeterminatorCondition.globalCondition)
            res = x.evaluate()
          Case DeterminatorRule.ConditionTypes.timeCondition
            Dim x As DeterminatorRule.DeterminatorCondition.timeCondition = CType(Rules(ruleIndex).Conditions(Counter).Condition, DeterminatorRule.DeterminatorCondition.timeCondition)
            res = x.evaluate()
          Case DeterminatorRule.ConditionTypes.x10Condition
            Dim x As DeterminatorRule.DeterminatorCondition.x10Condition = CType(Rules(ruleIndex).Conditions(Counter).Condition, DeterminatorRule.DeterminatorCondition.x10Condition)
            res = x.evaluate()
          Case DeterminatorRule.ConditionTypes.xPLMessage
            Dim x As DeterminatorRule.DeterminatorCondition.xplCondition = CType(Rules(ruleIndex).Conditions(Counter).Condition, DeterminatorRule.DeterminatorCondition.xplCondition)
            res = x.Evaluate(Message)
            If res And Not Initiated Then
              Initiated = True
            End If
        End Select
        If Rules(ruleIndex).MatchAny = False And res = False Then
          Exit Sub
        ElseIf res Then
          found = True
        End If
      ...
      If found And Initiated Then
        If xhcp.EnableLogging Then
          WriteErrorLog("Executing rule: " & Rules(ruleIndex).RuleName)
        End If
        ExecuteActions(ruleIndex)
      End If
    End Sub

    Private Sub ExecuteActions(ByVal ruleIndex As Integer)
      For Counter As Integer = 0 To Rules(ruleIndex).Actions.Length - 1
        If Not Rules(ruleIndex).Actions(Counter).Execute(Rules(ruleIndex).RuleName, Message) Then
          Exit For
        End If
      Next
    End Sub
  ...
  Private Shared Function ProcessSystemVariables(ByVal str As String) As String
    ' Date
